...
public class DynamicNames {
	...
	public String getLink(Sardine sardine, String serverAddress) throws IOException, com.test.webdav.FMFault_Exception, ParserConfigurationException, SAXException {
		 ...
		 */
		boolean asterisk_ex = true;
		while (asterisk_ex) {
			if (serverAddress.indexOf("*") > 0) {
				//take stack with dynamic name - it could be a folder or a file
				int asterisk = serverAddress.indexOf("*");
				String serverName_begin = serverAddress.substring(0, serverAddress.substring(0, asterisk).lastIndexOf("/")); //part of server name - before dynamic part
				String serverName_end = serverAddress.substring(asterisk).substring(serverAddress.substring(asterisk).indexOf("/") + 1); //part of server name - end dynamic part
				String serverName_dynamic = serverAddress.substring(serverName_begin.length(), serverAddress.length() - serverName_end.length()); //dynamic part

				//get all name in the directory serverName_begin
				List<DavResource> resources = sardine.list(serverName_begin);
	...
	private String getCorrectName(List<DavResource> resources, String dynamicName, String serverAddress) {
		String response = ""; //wrigth answer (if the program can't find address return "Error"
		boolean answer = true; //answer not found yet, if found - answer = false
		...
		String tempDynamicName = dynamicName; //Temp for template
		int minLength = 0; //How many symbols were used
		ArrayList<DavResource> tempList = new ArrayList<DavResource>();
		...
		while (answer) {//trying to find answer
			int asterisk = tempDynamicName.indexOf("*");
